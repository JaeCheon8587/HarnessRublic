#!/usr/bin/env python3
"""AI-Readiness Cartography — repo scorer (v3 rubric, 100 points, 6 categories).

Audits a repository against the 6-category AI-Ready rubric and emits structured
...
def score_a(apps: list[tt.App], root_claude: Path | None, overview_apps: list[str]) -> CategoryScore:
    ...
    if overview_only:
        pts = max(0, pts - 3 * len(overview_only))
        findings.append(
...
def score_b(
    ...
    if root_claude is None:
        ...
        sub["B1_RouterCompleteness"] = round(5 * ratio)
        if missing:
    ...
    if meta_pool:
        ...
        sub["B2_MetaHistory"] = round(5 * (meta_ratio + history_ratio) / 2)
    ...
    if rule_pool:
        ...
        sub["B3_FourStageMarkers"] = round(5 * ratio)
    ...
    if content_supporting:
        ...
        for p in content_supporting:
            ...
            if not missing:
                section_pass += 1
        if section_total:
            sub["B4_SectionCompliance"] = round(5 * section_pass / section_total)
    ...
    if frd_files:
        ...
        sub["B5_NoBlankViolations"] = round(5 * ratio)
...
# ----------------------------------------------------------------------------
# C. Cross-Module Dependency & Data Flow Mapping
# ----------------------------------------------------------------------------
def score_c(repo: Path, context_files: list[Path]) -> CategoryScore:
...
def score_d(repo: Path, context_files: list[Path]) -> CategoryScore:
    ...
    # D1 Reference accuracy: parse all path-like refs from context, verify existence
    total_refs = 0
    ...
    if total_refs == 0:
        sub["D1_RefAccuracy"] = 3  # neutral — nothing to verify
    else:
        accuracy = (total_refs - len(bad_refs)) / total_refs
        sub["D1_RefAccuracy"] = round(6 * accuracy)

    # D2 Critic / review infra
    ...
    if has_pr_template:
        d2 += 2
    sub["D2_CriticReview"] = d2

    # D3 Task validation commands actually exist
    ...
    if have_workflows:
        d3 += 1
    sub["D3_TaskValidation"] = min(5, d3)

    # D4 Prompt / agent eval tests
    has_evals = any((repo / p).exists() for p in ("evals", "benchmarks", "agent-evals", "prompts/test", "tests/agent"))
    sub["D4_PromptTests"] = 3 if has_evals else 0
...
def score_e(modules: list[Module], repo: Path) -> CategoryScore:
    ...
    pts = 0
    if measurable:
        # up to 8 pts for low drift
        pts += round(8 * (1 - drift_ratio))
...
# ----------------------------------------------------------------------------
# F. Agent Performance Outcomes
# ----------------------------------------------------------------------------
def score_f(repo: Path) -> CategoryScore:
...
def derive_actions(report_partial: dict[str, CategoryScore], modules: list[Module],
    ...
    actions: list[Action] = []
    A, B, C, D, E, F = (report_partial[k] for k in "ABCDEF")
    apps = apps or []
    ...
    # B1 — router 미흡
    if B.sub_scores.get("B1_RouterCompleteness", 0) < 4:
        actions.append(Action(
    ...
    # B2 — 메타 / 변경이력 누락
    if B.sub_scores.get("B2_MetaHistory", 0) < 4:
        actions.append(Action(
    ...
    # B3 — 4단계 마커 누락
    if B.sub_scores.get("B3_FourStageMarkers", 0) < 4:
        actions.append(Action(
    ...
    # B4 — 핵심 섹션 누락
    if B.sub_scores.get("B4_SectionCompliance", 0) < 4:
        actions.append(Action(
    ...
    if B.sub_scores.get("B5_NoBlankViolations", 0) < 4:
        actions.append(Action(
    ...
    # C — no architecture doc
    if not C.evidence.get("architecture_doc"):
        actions.append(Action(
    ...
    # D1 — broken refs
    if D.evidence.get("ref_broken", 0) > 0:
        actions.append(Action(
            title=f"context의 hallucinated path {D.evidence['ref_broken']}건 수정 (referential trust)",
            category="D",
            effort="S", effort_hours=0.5,
    ...
    if not E.evidence.get("ctx_validation_workflow") and not E.evidence.get("hook_validates_paths"):
        actions.append(Action(
...
def generate_insights(cats: dict[str, CategoryScore], total: int) -> list[str]:
    ...
    # D1 hallucination is special
    d = cats.get("D")
    if d and d.evidence.get("ref_broken", 0) > 0:
        out.append(
            f"⚠️  context에 hallucinated path {d.evidence['ref_broken']}건 — Meta 기준으로는 0이어야 함"
        )

    # E freshness
    e = cats.get("E")
    if e and e.evidence.get("drift_ratio", 0) > 0.3:
        out.append(f"context drift 높음 ({e.evidence['drift_ratio']:.0%}) — stale 위험")
...
def build_report(repo: Path) -> Report:
    ...
    context_files = find_all_context_files(repo)             # score_e 용
    ext_context_files = find_extended_context_files(repo)    # score_c/d 용
    ...
    cats = {
        ...
        "C": score_c(repo, ext_context_files),
        "D": score_d(repo, ext_context_files),
        "E": score_e(modules, repo),
        "F": score_f(repo),
    }
